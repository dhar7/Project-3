...
sys.path.append("/home/brk/development/pursec/mtlmine/miner/checker")
from checker import Event, check  # noqa: E402
...
def read_file(filename):
    ...
    while len(traces.keys()) < len(raw_traces[2]):
        ...
        traces[label] = list(
            map(lambda l: l[len(traces.keys())], raw_traces[1:]))
        i += j + 1
    ...
    for sensor_id, values in traces.items():
        ...
        if sensor_id == "gtyp_Interface_TXT_Controler.Subscribe.State_NFC_Device.History[19].i_code":
            continue
        ...
        for i, value in enumerate(values[1:]):
            ...
            raw_sequence.append(
                Event(
                    (datetime.strptime(time_ref[i], fstr) -
...
if __name__ == "__main__":
    ...
    results = check(read_file, args.events, args.formulas)
